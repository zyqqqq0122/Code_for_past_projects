...
r=cell(1,length(EbN0_db));
r0=cell(1,length(EbN0_db));
r1=cell(1,length(EbN0_db));
for snr_point = 1:length(EbN0_db)
  ...
  % Loop over several blocks to get sufficient statistics.
    for blk = 1:nr_blocks
        ...
        r{1,snr_point} = mf(t_samp:Q:t_samp+Q*(nr_training_bits+nr_data_bits)/2-1);

        % Phase estimation and correction.
        phihat = phase_estimation(r{1,snr_point}, b_train);
        r0{1,snr_point} = r{1,snr_point} * exp(-1i*phihat);
        r1{1,snr_point} = r{1,snr_point} * exp(-1i*pi*0.1);
