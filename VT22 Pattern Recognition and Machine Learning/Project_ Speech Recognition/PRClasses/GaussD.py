...
class GaussD:
    ...
    def prob(self, x):
        prob = np.exp(-0.5*np.dot(np.dot((x-self.means).reshape(1, -1), np.linalg.inv(self.cov)),
                                  (x-self.means).reshape(-1, 1))) \
               / ((2*math.pi)**(self.dataSize/2)*np.sqrt(np.linalg.det(self.cov)))
        return prob
